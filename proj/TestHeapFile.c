...
Attribute* getKeys(tableDescription *desc) {
	...
	Attribute *key = calloc(1, sizeof(Attribute));
	
	for (i = 0; i < numKeys; i++) {
		key->attName = calloc(strlen(attHead->attName) + 1, sizeof(char));
...
int main(int argc, char *argv[]) {
	...
	tableDescription *table = calloc(1, sizeof(tableDescription)); //description of table (after parsing)
	Buffer *buf = calloc(1, sizeof(Buffer)); //buffer object for page storage
